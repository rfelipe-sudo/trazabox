 ...
 * VERSIÓN CORREGIDA - RUT del técnico extraído correctamente
 */
  ...
  try {
    ...
    for (let i = 0; i < datos.length; i++) {
      ...
      produccion.push({
        ...
        rgu_total: resultado.rgu_total, // Alias para compatibilidad
    ...
    produccion.forEach(p => {
      ...
      tecnicosUnicos.add(p.tecnico);
      
      if (p.rut_tecnico && p.rut_tecnico !== "") {
        rutsProcesados.add(p.rut_tecnico);
      } else {
        rutsFaltantes.push(p.tecnico);
      }
    ...
    Logger.log("📊 Técnicos únicos: " + tecnicosUnicos.size);
    Logger.log("📊 RUTs procesados: " + rutsProcesados.size + " (" + Array.from(rutsProcesados).join(", ") + ")");
    
    if (rutsFaltantes.length > 0) {
      const unicosFaltantes = [...new Set(rutsFaltantes)];
      Logger.log("⚠️ Técnicos sin RUT (" + unicosFaltantes.length + "): " + unicosFaltantes.join(", "));
    }
    ...
    return { 
      ...
      tecnicos_unicos: tecnicosUnicos.size,
      ruts_procesados: rutsProcesados.size
    };
